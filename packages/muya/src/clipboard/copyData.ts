...
import { htmlToRtf } from './rtf/htmlToRtf';
...
// Cheap pre-filter before re-rendering the rich-text clipboard with MathML.
// False positives are harmless: the upgrade only changes the output of tokens
// the math extension actually tokenizes, so plain `$5 and $ 100` text is
// unaffected.
const MATH_LIKE_REG = /\$\$|\$[^$\n]+\$|\\\[|\\\(/;
...
export function getClipboardData(clipboard: Clipboard): IClipboardPayload {
    ...
    if (isSelectionInSameBlock) {
        ...
        // A selection inside a display-math block edits the bare LaTeX body —
        // the `$$` fences live in the block, not in the leaf text. Left as-is
        // the clipboard would carry literal TeX text; re-fence it so the html
        // slot renders standalone MathML (Word pastes a native equation) and
        // the plain slot carries the fenced markdown, matching what a
        // cross-block copy of the whole math block serializes (#14).
        if (text.length > 0 && anchorBlock.closestBlock?.('math-block') != null) {
            const fenced = `$$\n${text}\n$$`;
            return {
                html: getClipBoardHtml(fenced, {
                    ...options,
...
export function writeClipboardData(
    ...
    switch (copyType) {
        ...
        //
        // When the copied markdown contains formulas, the html slot renders
        // them as standalone MathML — Word / Word online convert MathML into
        // a native equation object on paste (marktext clipboard, #14: copy
        // formula → Office). Targets that ignore MathML still get the
        // surrounding text; the plain slot keeps the LaTeX source.
        //
        // Desktop Word ignores MathML inside the HTML slot (it dumps the
        // markup as literal text), so when the embedder supplies a
        // `writeRichClipboard` hook the same content is additionally built as
        // RTF — Word prefers RTF and converts its `{\mmath...}` groups into
        // native equations. The hook writes all three slots in one native
        // clipboard transaction and returns true; muya then leaves the event's
        // clipboardData untouched (Chromium cannot write RTF).
        case CopyType.COPY_AS_RICH: {
            if (text.length === 0)
                return;
            let clipboardHtml = html;
            let rtf = '';
            if (MATH_LIKE_REG.test(text)) {
                clipboardHtml = getClipBoardHtml(text, {
                    ...buildHtmlOptions(clipboard.muya.options ?? {}),
                    mathRenderMode: 'mathml',
                });
                try {
                    rtf = htmlToRtf(clipboardHtml);
                }
                catch {
                    rtf = '';
                }
            }
            const writeRichClipboard = clipboard.muya.options?.writeRichClipboard;
            if (rtf && typeof writeRichClipboard === 'function') {
                const handled = writeRichClipboard({ text, html: clipboardHtml, rtf });
                if (handled)
                    return;
            }

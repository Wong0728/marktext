...
describe('backspace at start-of-paragraph — unwrap block-quote / list', () => {
    ...
    it('lifts a MIDDLE list item OUT to a paragraph, splitting the list (no merge)', async () => {
        const muya = bootMuya('- one\n- two\n- three\n');
        ...
        await flush();
        const md = muya.getMarkdown();
        // `two` lifts OUT to a paragraph sitting between the two list halves
        // rather than being absorbed into `one` — nothing is dropped.
        expect(md).toContain('one');
        expect(md).toContain('two');
        expect(md).toContain('three');
        const state = muya.getState();
        // bullet-list(one) / paragraph(two) / bullet-list(three)
        expect(state.length).toBe(3);
        expect(state[0].name).toBe('bullet-list');
        expect(state[1].name).toBe('paragraph');
        expect(blockText(state, 1)).toBe('two');
        expect(state[2].name).toBe('bullet-list');
    });
});

...
class ParagraphContent extends Format {
    ...
    private _handleBackspaceInList() {
        ...
        // Backspace at a list item's start steps OUT one level per press: a
        // nested item outdents like Shift+Tab (level 3 -> 2 -> 1); a top-level
        // item then lifts OUT of the list into a plain paragraph (the branches
        // below, splitting the list as needed). Merging into the previous block
        // only happens on the next press, once it is an ordinary paragraph.
        const unindentType = this._getUnindentType();
        if (unindentType != null)
            return this._unindentListItem(unindentType);
        ...
        if (listItem.isOnlyChild()) {
        ...
        else if (listItem.isLastChild()) {
            // Last top-level item: lift its content out to sit AFTER the list,
            // leaving the earlier items untouched. A further Backspace on the
            // lifted paragraph then performs the normal paragraph merge.
            let anchor: Parent = list;
            listItem.forEach((node, i: number) => {
                const block = (node as Parent).clone() as Parent;
                list.parent!.insertAfter(block, anchor);
                anchor = block;
                if (i === 0)
            ...
            // Middle top-level item: split the list so the items before and
            // after stay intact, and this item's content is lifted out as
            // plain paragraph(s) in between (parallel to a leading heading /
            // paragraph) instead of merging into the previous item.
            const listAsList = list as TListBlock;
            const trailingState: IBulletListState | IOrderListState | ITaskListState
                = listAsList instanceof TaskList
                    ? { name: 'task-list', meta: { ...listAsList.meta }, children: [] }
                    : listAsList instanceof OrderList
                        ? { name: 'order-list', meta: { ...listAsList.meta }, children: [] }
                        : { name: 'bullet-list', meta: { ...listAsList.meta }, children: [] };

            const itemOffset = list.offset(listItem);
            list.forEachAt(itemOffset + 1, undefined, (node) => {
                if (node.isParent()) {
                    const childState = node.getState();
                    if (trailingState.name === 'task-list') {
                        if (isTaskListItemState(childState))
                            trailingState.children.push(childState);
                    }
                    else if (isListItemState(childState)) {
                        trailingState.children.push(childState);
                    }
            ...
            if (trailingState.children.length) {
                const trailingList = ScrollPage.loadBlock(trailingState.name).create(
                    this.muya,
                    trailingState,
                );
                list.parent!.insertAfter(trailingList, anchor);
            }

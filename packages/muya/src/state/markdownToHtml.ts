...
// First <h1> in the rendered article, used as the `<title>` fallback when the
// caller passes no explicit title. Tag-stripped so heading anchors / inline
// code spans don't leak into the title element.
const extractHeadingText = (html: string): string => {
    const match = /<h1[^>]*>([\s\S]*?)<\/h1>/i.exec(html);
    if (!match) return '';
    return unescapeHTML(match[1].replace(/<[^>]*>/g, '')).trim();
};

// Script detection good enough for a document-level `lang`: kana before han so
// Japanese text is not misread as Chinese, then hangul, then han → zh-CN.
const detectDocumentLanguage = (markdown: string): string => {
    if (/[\u3040-\u30ff]/.test(markdown)) return 'ja';
    if (/[\uac00-\ud7af]/.test(markdown)) return 'ko';
    if (/[\u4e00-\u9fff]/.test(markdown)) return 'zh-CN';
    return 'en';
};
...
export class MarkdownToHtml {
    ...
    private async _renderDiagram() {
        const selector
            = 'code.language-vega-lite, code.language-plantuml, code.language-flowchart, code.language-sequence, code.language-mindmap';
        ...
        for (const code of codes) {
            ...
            const functionType = (() => {
                if (/plantuml/.test(code.className))
                    ...
                    return 'sequence';
                else if (/mindmap/.test(code.className))
                    return 'mindmap';
                else
                    return 'vega-lite';
            })();
            ...
            try {
                ...
                else if (functionType === 'mindmap') {
                    diagramContainer.innerHTML = '';
                    render(diagramContainer, rawCode);
                }
            }
            catch {
    ...
    async generate(
        ...
        const docTitle = title || extractHeadingText(html);
        ...
        return `<!DOCTYPE html>
<html lang="${detectDocumentLanguage(this.markdown)}"${dirAttr}>
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>${sanitize(docTitle, EXPORT_DOMPURIFY_CONFIG, true)}</title>
${baseStyles}

...
export class MarkdownToState {
    ...
    private _buildCodeState(
        ...
        const diagramMatch = /^(mermaid|vega-lite|plantuml|flowchart|sequence|mindmap)$/.exec(lang);
        if (diagramMatch) {
            const diagramType = diagramMatch[1] as 'mermaid' | 'vega-lite' | 'plantuml' | 'flowchart' | 'sequence' | 'mindmap';
            return {

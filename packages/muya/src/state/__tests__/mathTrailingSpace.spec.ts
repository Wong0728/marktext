...
describe('block math — closing $$ with trailing whitespace (#1931)', () => {
    ...
    it('parses CRLF display math without swallowing the following blocks', () => {
        const states = parse('$$\r\nx = 1\r\n$$\r\n\r\n## after\r\n');
        expect(states.filter(s => s.name === 'math-block')).toHaveLength(1);
        expect(states.some(s => s.name === 'atx-heading')).toBe(true);
    });

    it('keeps display math after a block quote out of the quote continuation', () => {
        const states = parse('> hint\r\n$$\r\nx = 1\r\n$$\r\n');
        expect(states.map(s => s.name)).toEqual(['block-quote', 'math-block']);
    });

    it('keeps display math after paragraph text out of the paragraph continuation', () => {
        const states = parse('therefore\r\n$$\r\nx = 1\r\n$$\r\n');
        expect(states.map(s => s.name)).toEqual(['paragraph', 'math-block']);
    });
});

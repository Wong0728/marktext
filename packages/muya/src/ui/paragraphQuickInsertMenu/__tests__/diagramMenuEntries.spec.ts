...
// Lock the quick-insert menu's diagram section to all supported diagram
// engines. flowchart + sequence were restored for parity with the legacy
// muyajs engine; mindmap was added via markmap. A future refactor of the menu
// config shouldn't silently drop them.
describe('quick-insert menu — diagram entries', () => {
    ...
    it('exposes the mindmap insert entry', () => {
        const labels = diagramSection!.children.map(child => child.label);
        expect(labels).toContain('diagram mindmap');
    });

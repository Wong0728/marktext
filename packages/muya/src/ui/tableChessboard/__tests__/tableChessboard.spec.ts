...
describe('tableChessboard — plugin shape (restored, revert #4435)', () => {
    it('is exported from the package entrypoint', async () => {
        // Dynamic-importing the full package entrypoint triggers a one-time
        // TypeScript transform of every source file, which can take several
        // seconds — especially under CI / full-suite load. Allow ample headroom
        // over the default 5 s so this is not a flaky timeout.

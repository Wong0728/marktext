...
export interface IMuyaOptions {
    ...
    /**
     * Write a rich-text clipboard payload natively on behalf of the embedder.
     *
     * Invoked by "Copy as Rich Text" when the copied content contains
     * formulas: `rtf` carries the rendered content with OMML equation groups,
     * which desktop Word converts into native equations on paste (its HTML
     * paste filter ignores MathML). Chromium's `clipboardData` cannot write
     * RTF, so the embedder must write all three slots in one native clipboard
     * transaction (e.g. Electron `clipboard.write`). Return `true` when the
     * clipboard was written and muya should skip `clipboardData.setData`
     * entirely; return `false` to fall back to the default HTML/plain slots.
     */
    writeRichClipboard?: (payload: { text: string; html: string; rtf: string }) => boolean;
    /**

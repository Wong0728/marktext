...
// LaTeX `\(...\)` inline math. Must run before `tryBacklash`, otherwise the
// leading `\(` is consumed as a backslash escape and the math rule never fires.
// Produces the same `inline_math` token shape as the `$...$` path in
// `tryChunks`, so the existing renderer handles it unchanged.
function tryInlineMathParen(state: ILexState): boolean {
    const to = state.inlineRules.inline_math_paren.exec(state.src);
    if (!to || !isLengthEven(to[2]))
        return false;
...
function tryInlineMathBracket(state: ILexState): boolean {
    const to = state.inlineRules.inline_math_bracket.exec(state.src);
    if (!to || !isLengthEven(to[2]))
        return false;

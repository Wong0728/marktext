...
export const inlineExtensionRules = {
    // eslint-disable-next-line regexp/no-super-linear-backtracking
    inline_math: /^(\$)((?:[^$\\]|\\.)+)(\\*)\1(?!\1)/,
    // LaTeX `\(...\)` inline delimiter (single-line). Consumed by a dedicated
    // lexer handler that runs before `backlash`, otherwise `\(` is eaten as an
    // escape before the math rule ever sees it.
    // eslint-disable-next-line regexp/no-super-linear-backtracking
    inline_math_paren: /^\\\(((?:[^\\\n]|\\.)+?)(\\*)\\\)/,
    // LaTeX `\[...\]` inline display delimiter. Single-line, same precedence as
    // `\(...\)`.
    // eslint-disable-next-line regexp/no-super-linear-backtracking
    inline_math_bracket: /^\\\[((?:[^\\\n]|\\.)+?)(\\*)\\\]/,

...
export const EXPORT_DOMPURIFY_CONFIG = {
    ...
    ALLOW_DATA_ATTR: false,
    ADD_ATTR: ['data-align'],
    // Keep KaTeX's `<semantics><annotation encoding="application/x-tex">` pair
    // intact. Stripping `<semantics>` instead unwraps it and leaves the raw TeX
    // as a stray text node directly inside `<math>`, which browsers and pandoc
    // both read as math content. `<annotation-xml>` stays forbidden — it is the
    // MathML script-injection vector and KaTeX never emits it.
    ADD_TAGS: ['semantics', 'annotation'],
    USE_PROFILES: {
        html: true,
        svg: true,
        svgFilters: true,
        // Preserve MathML so KaTeX's <math> element (inside .katex-mathml)
        // survives sanitization. Without this, DOMPurify strips <math> and
        // pandoc's HTML→DOCX converter has no semantic math to turn into OMML.
        mathMl: true,
    },
    RETURN_TRUSTED_TYPE: false,

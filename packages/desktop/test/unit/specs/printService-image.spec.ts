...
describe('resolveLocalImageSrc — branch coverage', () => {
  ...
  it('(b) Windows drive image path → file:///C:/… (drive colon stays in the path)', () => {
    // `file://C:\…` would parse `C:` as the URL authority (host) — the exact
    // malformed form the export fix removed (pandoc dropped such images).
    expect(resolveLocalImageSrc('C:\\pics\\b.png')).toBe('file:///C:/pics/b.png')
  })

  it('(c) UNC image path → file://host/share/… (host is the URL authority)', () => {
  ...
  it('absolute path with a query keeps its extension recognised (POSIX → file://)', () => {
    // `.png` immediately followed by `?` still matches IMAGE_EXT_REG, and the
    // `?` is percent-encoded: raw in a URL it would start the query string and
    // truncate the path.
    expect(resolveLocalImageSrc('/tmp/b.png?v=2')).toBe('file:///tmp/b.png%3Fv%3D2')
  })

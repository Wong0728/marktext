...
// Drive the whole test with POSIX path semantics so the POSIX-style fixtures
// (e.g. '/tmp/notes/assets') behave identically on every OS. On Windows, node's
// default win32 `path.resolve` injects a drive letter, which breaks
// moveImageToFolder's `noHashPath === imagePath` short-circuit and the
// separator-based assertions below.

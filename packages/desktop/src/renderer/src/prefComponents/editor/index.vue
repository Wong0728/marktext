  ...
  <div class="pref-editor">
    ...
    <compound>
      ...
      <template #children>
        ...
        />
        <bool
          :description="t('preferences.editor.writingBehavior.vimMode')"
          :bool="vimMode"
          :on-change="(value) => onSelectChange('vimMode', value)"
        />
        <text-box
          :description="t('preferences.editor.writingBehavior.writingTarget')"
          :input="writingTarget === 0 ? '' : String(writingTarget)"
          :emit-time="0"
          default-value="0"
          :on-change="onWritingTargetChange"
        />
      </template>
    </compound>
...
// The writing goal is a free-form number field; empty / invalid input means
// "no target" (0). Clamped to a sane range so a typo can't break the UI.
const onWritingTargetChange = (value: unknown): void => {
  const parsed = Math.floor(Number(value))
  const target = Number.isFinite(parsed) && parsed > 0 ? Math.min(parsed, 10_000_000) : 0
  preferenceStore.SET_SINGLE_PREFERENCE({ type: 'writingTarget', value: target })
}
</script>

...
const onFileContextmenu = (event: MouseEvent): void => {
...
onMounted(() => {
  if (fileEl.value) {
    fileEl.value.addEventListener('contextmenu', onFileContextmenu)
  }
...
onBeforeUnmount(() => {
  if (fileEl.value) {
    fileEl.value.removeEventListener('contextmenu', onFileContextmenu)
  }

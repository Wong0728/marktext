    ...
    >
      ...
      >
        <input
          ...
          type="text"
          class="filter-input"
          :placeholder="t('sideBar.tree.filterFiles')"
          @keydown.esc.stop="nameFilter = ''"
        >
        <folder
        ...
        />
        <div
          v-if="isFilterActive && !rootHasMatches"
          class="empty-project"
        >
          <span>{{ t('sideBar.tree.noMatchingFiles') }}</span>
        </div>
        <div
          v-else-if="
...
// #6 — render-layer filename filter for the project tree. Typing narrows the
// tree to matching names (subtree stays visible when its folder name matches
// itself); Escape clears the box. The store's tree is never mutated.
const nameFilter = ref('')
const normalizedFilter = computed(() => normalizeFilter(nameFilter.value))
const isFilterActive = computed(() => normalizedFilter.value.length > 0)
// The root folder's own name matching means the whole (unfiltered) subtree
// is shown, matching the folder-level rule in treeFolder.vue.
const showWholeRoot = computed(() =>
  isFilterActive.value && !!props.projectTree?.name.toLowerCase().includes(normalizedFilter.value)
)
const visibleRootFolders = computed(() => {
  const tree = props.projectTree
  if (!tree || !isFilterActive.value || showWholeRoot.value) return tree?.folders ?? []
  return tree.folders.filter(folder => folderMatchesFilter(folder, normalizedFilter.value))
})
const visibleRootFiles = computed(() => {
  const tree = props.projectTree
  if (!tree || !isFilterActive.value || showWholeRoot.value) return tree?.files ?? []
  return tree.files.filter(file => fileMatchesFilter(file, normalizedFilter.value))
})
const rootHasMatches = computed(() => {
  const tree = props.projectTree
  if (!tree) return false
  if (!isFilterActive.value) return true
  return visibleRootFolders.value.length > 0 || visibleRootFiles.value.length > 0
})
...
const onDocumentContextmenu = (event: MouseEvent): void => {
  const target = event.target as HTMLElement | null
  if (target && target.tagName !== 'INPUT') {
    projectStore.createCache = {}
    projectStore.renameCache = null

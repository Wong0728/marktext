...
let dragBarMouseDownHandler: ((event: MouseEvent) => void) | null = null
...
onMounted(() => {
  nextTick(() => {
    ...
    dragBarMouseDownHandler = mouseDownHandler
  })
})

onBeforeUnmount(() => {
  if (dragBar.value && dragBarMouseDownHandler) {
    dragBar.value.removeEventListener('mousedown', dragBarMouseDownHandler, false)
    dragBarMouseDownHandler = null
  }
})

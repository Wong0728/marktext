    ...
    :class="[
      {
        ...
        focus: focus,
        source: sourceCode,
        'show-heading-numbers': showHeadingNumbers
      }
    ]"
    :dir="textDirection"
  >
...
import {
  exportStyledHTML,
  prepareHtmlForDocx,
  prepareHtmlForJsDocx,
  embedImagesAsDataUris,
  collectLocalImagesForZip,
...
// Cached pandoc availability — queried once per session so the 'auto' DOCX
// engine mode does not round-trip to the main process on every export.
let cachedPandocExists: boolean | null = null
const isPandocInstalled = async (): Promise<boolean> => {
  if (cachedPandocExists === null) {
    cachedPandocExists = await window.electron.ipcRenderer.invoke('mt::pandoc::exists')
  }
  return cachedPandocExists
}
...
const handleExport = async (options: unknown) => {
  ...
  try {
    ...
    switch (type) {
      case 'docx': {
      // DOCX export is handled by pandoc or the JS engine in the main
      // process. We send the styled HTML that the PDF/HTML exporters use,
      // with engine-specific preprocessing applied so each engine gets the
      // HTML shape it reads best:
      //   - pandoc: strip KaTeX visual spans, keep <math> MathML → OMML
      //   - JS:     strip MathML, keep KaTeX visual spans → styled text
        try {
          const rawHtml = await exportStyledHTML(editor.value, markdown, {
            title: htmlTitle || '',
          ...
          const engine = (opts.docxEngine || 'auto') as 'auto' | 'pandoc' | 'js'
          // 'auto' resolves the same way the main process does: prefer pandoc
          // (for native OMML equations) when installed, else fall back to JS.
          const useJs =
          engine === 'js' || (engine === 'auto' && !(await isPandocInstalled()))
          let content: string
          let mathBlocks: MathBlock[] = []
          if (useJs) {
            const result = prepareHtmlForJsDocx(rawHtml)
            content = result.html
            mathBlocks = result.mathBlocks
          } else {
            content = await prepareHtmlForDocx(rawHtml)
          }
          editorStore.EXPORT({
            type,
            content,
            docxEngine: engine,
            mathBlocks,
            docxFont: opts.docxFont
          })
        } catch (err) {
          cachedPandocExists = null
      ...
      case 'styledHtml': {
        try {
          let content = await exportStyledHTML(editor.value, markdown, {
            title: htmlTitle || '',
            printOptimization: false,
            extraCss,
            toc: htmlToc,
          ...
          if (opts.exportZip) {
          // Portable archive: rewrite local images to relative `images/…`
          // paths and let the main process pack them next to the HTML.
          // Remote images keep their original URLs.
            const { html, images } = await collectLocalImagesForZip(content)
            editorStore.EXPORT({ type, content: html, zipImages: images })
          } else {
            if (opts.embedImages) {
            // Self-contained HTML: inline local images as data URIs so the file
            // keeps rendering after it is moved or shared on its own.
              content = await embedImagesAsDataUris(content)
            }
      ...
      // NOTE: We need to set page size via Electron.
        try {
          ...
          // The full document is handed to main, which rasterizes it in a
          // dedicated hidden window (#11) — the live editor DOM is no longer
          // part of the PDF path.
          editorStore.EXPORT({ type, pageOptions, content: html })
        } catch (err) {
          log.error('Failed to export document:', err)
          notice.notify({
...
const handleInlineFormat = (type: unknown) => {
  if (sourceCode.value) {
    // Keybinding parity (#2): the same format accelerators work in
    // source-code mode by wrapping the selection with markdown markers
    // (sourceCode.vue listens on `cm-format`).
    bus.emit('cm-format', type)
    return
  }
...
onMounted(() => {
  ...
  const options: Record<string, unknown> = {
    ...
    // "Copy as Rich Text" with formulas: write markdown + HTML + RTF natively
    // (Chromium's clipboardData has no RTF slot; Word turns RTF OMML groups
    // into native equations on paste). Returning true makes the engine skip
    // the event's clipboardData writes.
    writeRichClipboard: (payload: { text: string; html: string; rtf: string }) => {
      window.electron.clipboard.writeRich(payload)
      return true
    },
  ...
  editor.value.on('selection-change', (changes: MuyaChange) => {
    ...
    // Selection word count (#2791): exact within a block, best-effort across
    // blocks; cheap enough to recompute on every selection change.
    if (editor.value) {

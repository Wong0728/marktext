import { pathToFileUrl } from './fileUrl'
...
export function resolveLocalLinkHref(href: string): string {
  ...
  if (/^[a-z]:[\\/]/i.test(href)) return pathToFileUrl(href)
  // POSIX / UNC absolute path → file://.
  if (/^(?:\/|\\\\)/.test(href)) return pathToFileUrl(href)
  ...
  if (window.DIRNAME) return pathToFileUrl(window.path.resolve(window.DIRNAME, href))
  return href
}

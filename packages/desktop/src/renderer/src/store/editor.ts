...
interface ExportPayload {
  ...
  /** Which DOCX engine the main process should use ('auto' | 'pandoc' | 'js'). */
  docxEngine?: 'auto' | 'pandoc' | 'js'
  /** User's DOCX font settings (latin / east-asian font, body size). */
  docxFont?: { latin?: string; eastAsia?: string; sizePt?: number }
  ...
  /** Local image bytes for ZIP export, packed next to the HTML by main. */
...
export interface EditorState {
  ...
  listToc: TocItem[]
  toc: TocTreeNode[]
  // Word/character counts of the active text selection (null when the caret
  // has no selection). Driven from muya `selection-change` (#2791).
  selectionWordCount: { word: number; character: number; paragraph: number } | null
}
...
export const useEditorStore = defineStore('editor', {
  ...
  actions: {
    SET_SELECTION_WORD_COUNT(
      count: { word: number; character: number; paragraph: number } | null
    ): void {
      this.selectionWordCount = count
    },
    ...
    RESTORE_UNSAVED_TABS(rawTabs: unknown): void {
      if (!Array.isArray(rawTabs) || rawTabs.length === 0) {
        return
      }

      let firstRestored: IFileState | null = null
      for (const raw of rawTabs) {
        const tabData = raw as Partial<IFileState> & Record<string, unknown>

        // A tab for the same file may already be open (e.g. the startup path
        // opened it from disk). Replace its content with the recovered, newer
        // unsaved version instead of duplicating the tab.
        const existingTab = tabData.pathname
          ? this.tabs.find((t) => window.fileUtils.isSamePathSync(t.pathname, tabData.pathname!))
          : undefined
        if (existingTab) {
          existingTab.markdown =
            typeof tabData.markdown === 'string' ? tabData.markdown : existingTab.markdown
          existingTab.isSaved = false
          // Recovered content differs from disk, so there is no clean baseline:
          // never let an undo mark it saved (see LISTEN_FOR_CONTENT_CHANGE).
          existingTab.savedMarkdown = undefined
          existingTab.lastSavedHistoryId = -1
          firstRestored = firstRestored ?? existingTab
          continue
        }

        const fileState = createDocumentState(tabData)
        fileState.isSaved = false
        fileState.savedMarkdown = undefined
        fileState.lastSavedHistoryId = -1
        this.tabs.push(fileState)
        firstRestored = firstRestored ?? fileState
      }

      this.updateTabIdToIndex()

      if (firstRestored) {
        const { id, markdown, cursor } = firstRestored
        this.UPDATE_CURRENT_FILE(firstRestored)
    ...
    LISTEN_FOR_SET_PATHNAME(): void {
      window.electron.ipcRenderer.on('mt::set-pathname', (_, fileInfo) => {
        ...
        if (tab) {
          // Renaming via Save As makes the new path the on-disk baseline for
          // source-mode dirty tracking (see LISTEN_FOR_CONTENT_CHANGE).
          Object.assign(tab, { filename, pathname, isSaved: true, savedMarkdown: tab.markdown })
          debouncedSendBufferedState()
        }
      })
      ...
      window.electron.ipcRenderer.on('mt::tab-saved', (_, tabId) => {
        ...
        if (tab) {
          ...
          // The just-saved content becomes the clean baseline for source-mode
          // dirty tracking (WYSIWYG uses lastSavedHistoryId above).
    ...
    LISTEN_FOR_CLOSE(): void {
      ...
      window.electron.ipcRenderer.on('mt::ask-for-close', () => {
        // Acknowledge synchronously so the main process knows this renderer is
        // responsive. If it were wedged in a JS loop this would never run and
        // main falls back to a force-close prompt (H2).
        window.electron.ipcRenderer.send('mt::close-request-ack')
        sendBufferedState()
          .catch((err) => {
          ...
          .then(() => {
            ...
            if (unsavedFiles.length) {
              // Always ask: the user's explicit choice is the only authority
              // over unsaved content.
    ...
    UPDATE_CURRENT_FILE(currentFile: IFileState): void {
      ...
      if (oldCurrentFile == null || oldCurrentFile.id !== currentFile.id) {
        // The outgoing tab's selection count is meaningless for the new tab.
    ...
    // Close a set of tabs with a single save/confirm dialog for the unsaved
    // ones (M7). Closing "other"/"all" tabs previously fired one dialog per
    // unsaved tab; saved tabs are force-closed immediately.
    CLOSE_TABS_BATCHED(tabsToClose: IFileState[]): void {
      if (!tabsToClose.length) return
      const projectStore = useProjectStore()

      const savedTabIds = tabsToClose.filter((f) => f.isSaved).map((f) => f.id)
      if (savedTabIds.length) {
    ...
    CLOSE_OTHER_TABS(file: IFileState): void {
      this.CLOSE_TABS_BATCHED(this.tabs.filter((f) => f.id !== file.id))
    },
    ...
    CLOSE_ALL_TABS(): void {
      this.CLOSE_TABS_BATCHED(this.tabs.slice())
    },
    ...
    /**
     * Move a tab to a given index (tab bar drag & drop). `toIndex` is the
     * insertion index within the tab list *without* the moved tab.
     */
    MOVE_TAB_TO_INDEX(move: { fromId: string; toIndex: number }): void {
      const { fromId, toIndex } = move
      const { tabs } = this
      const fromIndex = tabs.findIndex((t) => t.id === fromId)
      if (fromIndex === -1) return

      const [item] = tabs.splice(fromIndex, 1)
      const targetIndex = Math.max(0, Math.min(toIndex, tabs.length))
    ...
    LISTEN_FOR_CONTENT_CHANGE({
      ...
      const isDirty =
        history === undefined
          ? markdown !== (tab.savedMarkdown ?? oldMarkdown)
          : historyMarksDirty
      if (isDirty) {
        tab.isSaved = false
        ...
        // Source-code mode reports no undo-history id, so matching the on-disk
        // baseline (`savedMarkdown`) is the only clean-state signal: clear the
        // dirty flag when an edit is undone back to the saved text. Skip when no
        // baseline is known (e.g. a crash-recovered unsaved tab) so recovered
        // content is never mistaken for saved.
        if (tab.savedMarkdown !== undefined) {
          tab.isSaved = true
        }
    ...
    HANDLE_AUTO_SAVE({ id, pathname }: AutoSavePayload): void {
      ...
      const timer = setTimeout(() => {
        ...
        // Read path/filename/options from the live tab, not the closure: a
        // rename or Save As inside the autosave delay window would otherwise
        // write the content back to the stale path, recreating the old file.
        if (tab && !tab.isSaved && tab.pathname) {
          const defaultPath = getRootFolderFromState(projectStore)
          window.electron.ipcRenderer.send(
            'mt::response-file-save',
            id,
    ...
    EXPORT({ type, content, pageOptions, docxEngine, docxFont, mathBlocks, zipImages }: ExportPayload): void {
    ...
    SET_LINE_ENDING(lineEnding: LineEnding | string): void {
      ...
      if (lineEnding !== oldLineEnding) {
        ...
        // Changing the line ending alters the bytes written to disk, so the tab
        // is now dirty — marking it saved here would drop the change from both
        // the save prompt and the crash snapshot.
    ...
    LISTEN_FOR_SET_ENCODING(): void {
      bus.on('mt::set-file-encoding', (encodingName) => {
        ...
        if (encoding !== encodingName) {
          ...
          // Encoding affects the on-disk bytes: mark dirty, not saved.
          this.currentFile.isSaved = false
          debouncedSendBufferedState()
        }
      })
    ...
    LISTEN_FOR_SET_FINAL_NEWLINE(): void {
      bus.on('mt::set-final-newline', (value) => {
        ...
        if (trimTrailingNewline !== value) {
          ...
          // The trailing-newline option changes the saved file: mark dirty.
          this.currentFile.isSaved = false
          debouncedSendBufferedState()
        }
      })
    ...
    LISTEN_FOR_RESTORE_UNSAVED_TABS(): void {
      window.electron.ipcRenderer.on('mt::restore-unsaved-tabs', (_, tabs) => {
        this.RESTORE_UNSAVED_TABS(tabs)
      })
    }
  }
})
...
const trimTrailingNewlines = (text: string): string => {
  // Character class must be CR/LF only. `[\r?\n]` also matches a literal `?`,
  // which silently ate a trailing question mark (`"what?"` -> `"what"`).
  return text.replace(/[\r\n]+$/, '')
}

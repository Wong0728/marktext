...
export interface IpcInvokeChannels {
  ...
  // Whether pandoc is installed. Uses the same lookup as the export path
  // (MARKTEXT_PANDOC env, PATH, and Windows install-dir fallback), so the
  // renderer can resolve the 'auto' DOCX engine the same way the main
  // process does.
  'mt::pandoc::exists': { args: []; ret: boolean }
  ...
  // Tab drag & drop across windows: the window under the cursor accepts a
  // dropped tab (top strip only) and asks main to open it there. Returns
  // false when the tab could not be adopted (source keeps its tab).
  'mt::tab-drag-adopt': { args: [payload: TabDragPayload]; ret: boolean }
  // Sent by the source window when a tab drag ends without an in-window
  // drop. Main decides: tab was adopted elsewhere / merge into the window
  // under the cursor / create a new window / cancel. `adopted: true` means
  // the source window must close the tab.
  'mt::tab-drag-finished': {
    args: [dragId: string, payload: TabDragPayload | null]
    ret: { adopted: boolean }
  }
  // "Open in new window" tab context-menu action: moves the tab into a
  // freshly created editor window.
  'mt::move-tab-to-new-window': { args: [payload: TabDragPayload]; ret: boolean }
...
export interface IpcSendChannels {
  ...
  'mt::clipboard::write-rich': [payload: { text: string; html: string; rtf: string }]
  'mt::close-window': []
  'mt::close-window-confirm': [unsavedFiles: UnsavedFile[]]
  // Synchronous liveness ack for `mt::ask-for-close`; lets main tell a wedged
  // renderer apart from a slow one (see windows/editor.ts close handling).
  'mt::close-request-ack': []
  'mt::cmd-close-window': []
  'mt::cmd-import-file': []
...
export interface IpcMainEventChannels {
  ...
  'mt::restore-unsaved-tabs': [tabs: unknown[]]

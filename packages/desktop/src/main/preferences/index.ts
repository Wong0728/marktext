...
class Preference extends TypedEmitter<PreferenceEvents> {
  ...
  constructor(paths: AppPaths) {
    ...
    // `restoreAll` was replaced by the crash-recovery snapshot mechanism.
    // The value is kept in the schema enum so old preference files still
    // validate, but it is normalized away on every start.
    if (this.store.get('startUpAction') === 'restoreAll') {
      this.store.set('startUpAction', 'openLastFolder')
    }

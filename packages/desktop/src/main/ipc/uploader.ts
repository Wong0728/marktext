...
const uploadByCli = (cliScript: string, localPath: string): Promise<string> =>
  new Promise((resolve, reject) => {
    // `cliScript` arrives from the renderer on every request. Require an
    // existing file so a compromised renderer can't coax MarkText into
    // launching an arbitrary PATH-resolved command (execFile already avoids a
    // shell, so only the program itself — not its args — is the risk).
    if (typeof cliScript !== 'string' || !cliScript.trim() || !fs.pathExistsSync(cliScript)) {
      return reject(new Error('Invalid CLI uploader script path'))
    }
    execFile(
      cliScript,
      [localPath],

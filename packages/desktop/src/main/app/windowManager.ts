...
interface EditorBufferStoreLike {
  deleteSnapshot(restoreBufferId: string | undefined): void
}
...
class WindowManager extends TypedEmitter<WindowManagerEvents> {
  ...
  remove(windowId: number): BaseWindow | undefined {
    ...
    if (window) {
      ...
      // Drop the per-window application menu here too, not only in forceClose:
      // the crash "Close" path destroys the window (to keep its recovery
      // snapshot) and reaches remove() via 'window-closed' without ever calling
      // forceClose, which previously leaked the menu map entry. Idempotent.
  ...
  forceClose(browserWindow: IBrowserWindow | null | undefined): boolean {
    ...
    // A clean close means the user has already decided about unsaved content
    // (saved or discarded) — drop the crash-recovery snapshot unconditionally.
    this.editorBufferStore.deleteSnapshot(
  ...
  private _listenForIpcMain(): void {
    ...
    ipcMain.on('mt::close-window', (e) => {
      const win = BrowserWindow.fromWebContents(e.sender)
      // forceClose() deletes the window's crash-recovery snapshot.
      this.forceClose(win)
    })

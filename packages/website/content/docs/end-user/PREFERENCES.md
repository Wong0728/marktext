...
| texMathDollars      | Boolean | `true`  | Enable pandoc's [`tex_math_dollars`](https://pandoc.org/MANUAL.html#extension-tex_math_dollars) extension — `$…$` and `$$…$$` math.                                                                                               |
| texMathGfm          | Boolean | `false` | Enable pandoc's [`tex_math_gfm`](https://pandoc.org/MANUAL.html#extension-tex_math_gfm) extension — GitHub's ``$`…`$`` and ` ```math ` math. Replaces the former `isGitlabCompatibilityEnabled`, which is migrated automatically. |

...
try {
  ...
  // patch-package aborts if the patch is already applied (re-install on an
  // already-patched tree). Treat an already-patched binding.gyp as success.
  const gyp = path.join(nativeKeymapDir, 'binding.gyp')
  const alreadyPatched =
    fs.existsSync(gyp) && fs.readFileSync(gyp, 'utf8').includes('stdcpp20')
  if (!alreadyPatched) throw e
  console.log('native-keymap patch already applied; continuing.')
}
